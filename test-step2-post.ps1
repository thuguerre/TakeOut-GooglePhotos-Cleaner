<#
.SYNOPSIS
    Verify if all empty folders have well been deleted.
...
if (-Not (Test-Path $takeOutArchivePath"\file-1-to-ignore.txt")) { exit 1 }
    ...
    # folder 2
    if (Test-Path $takeOutArchivePath"\folder-2-to-delete") { exit 1 }

        # folder 2.1
        if (Test-Path $takeOutArchivePath"\folder-2-to-delete\folder-2.1-to-delete") { exit 1 }
    ...
    if (-Not (Test-Path $takeOutArchivePath"\folder-3\file-3.1-to-ignore.txt")) { exit 1 }
        ...
        # folder 3.1
        if (Test-Path $takeOutArchivePath"\folder-3\folder-3.1-to-delete") { exit 1 }
    ...
    # folder 6.2
    if (-Not (Test-Path $takeOutArchivePath"\folder-6.2")) { exit 1 }
    if (-Not (Test-Path $takeOutArchivePath"\folder-6.2\file-6.2.1-to-synchronize.txt")) { exit 1 }
